...
def _check_zero(result):
    if result != 0:
        raise InotifyError("Call failed (should return zero): (%d)" % (result,))
...
def _check_nonzero(result):
    if result == 0:
        raise InotifyError("Call failed (should return nonzero): (%d)" % (result,))
...
def _check_nonnegative(result):
    if result == -1:
        raise InotifyError("Call failed (should not be -1): (%d)" % (result,))
